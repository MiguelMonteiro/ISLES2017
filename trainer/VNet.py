...
def v_net(tf_input, n_channels):
    ...
    with tf.variable_scope('decoder_level_4'):
        ...
        with tf.variable_scope('convolution_1'):
            d41 = tf.nn.relu(convolution_layer_3d(d4, [5, 5, 5, n_channels * 16, n_channels * 8], [1, 1, 1, 1, 1]))
        with tf.variable_scope('convolution_2'):
            d41 = tf.nn.relu(convolution_layer_3d(d41, [5, 5, 5, n_channels * 8, n_channels * 8], [1, 1, 1, 1, 1]))
        with tf.variable_scope('convolution_3'):
            d41 = tf.nn.relu(convolution_layer_3d(d41, [5, 5, 5, n_channels * 8, n_channels * 8], [1, 1, 1, 1, 1]))

        d42 = d41 + c52
        with tf.variable_scope('up_sample'):
            d43 = tf.nn.relu(deconvolution_layer_3d(d42, [2, 2, 2, n_channels * 4, n_channels * 8], tf.shape(c31), [1, 2, 2, 2, 1]))
    ...
    with tf.variable_scope('decoder_level_3'):
        ...
        with tf.variable_scope('convolution_1'):
            d31 = tf.nn.relu(convolution_layer_3d(d3, [5, 5, 5, n_channels * 8, n_channels * 4], [1, 1, 1, 1, 1]))
        with tf.variable_scope('convolution_2'):
            d31 = tf.nn.relu(convolution_layer_3d(d31, [5, 5, 5, n_channels * 4, n_channels * 4], [1, 1, 1, 1, 1]))
        with tf.variable_scope('convolution_3'):
            d31 = tf.nn.relu(convolution_layer_3d(d31, [5, 5, 5, n_channels * 4, n_channels * 4], [1, 1, 1, 1, 1]))

        d32 = d31 + d43
        with tf.variable_scope('up_sample'):
            d33 = tf.nn.relu(deconvolution_layer_3d(d32, [2, 2, 2, n_channels * 2, n_channels * 4], tf.shape(c21), [1, 2, 2, 2, 1]))

    with tf.variable_scope('decoder_level_2'):
        d2 = tf.concat((d33, c21), axis=-1)
        with tf.variable_scope('convolution_1'):
            d21 = tf.nn.relu(convolution_layer_3d(d2, [5, 5, 5, n_channels * 4, n_channels * 2], [1, 1, 1, 1, 1]))
        with tf.variable_scope('convolution_2'):
            d21 = tf.nn.relu(convolution_layer_3d(d21, [5, 5, 5, n_channels * 2, n_channels * 2], [1, 1, 1, 1, 1]))

        d22 = d21 + d33
        with tf.variable_scope('up_sample'):
            d23 = tf.nn.relu(deconvolution_layer_3d(d22, [2, 2, 2, n_channels, n_channels * 2], tf.shape(c11), [1, 2, 2, 2, 1]))

    with tf.variable_scope('decoder_level_1'):
        # up-path level 2
        d1 = tf.concat((d23, c11), axis=-1)
        with tf.variable_scope('convolution_1'):
            d11 = tf.nn.relu(convolution_layer_3d(d1, [5, 5, 5, n_channels * 2, n_channels], [1, 1, 1, 1, 1]))
        d12 = d11 + d23
